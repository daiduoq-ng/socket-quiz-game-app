...
namespace NT106
{
    public partial class HomePage : Form
    {
        ...
        private void btnEvent_Click(object sender, EventArgs e)
        {
            ...
            Form_Event formEvent = new Form_Event(this, questionJson,username);
            ...
            try
            {
                while (true)
                {
                    ...
                    if (bytesRead > 0)
                    {
                        ...
                        // Chuyển đổi dữ liệu từ byte array sang chuỗi
                        var jsonObject = JsonConvert.DeserializeObject<dynamic>(questionJson);

                        if (jsonObject.status == "success")
                        {
                            // Gọi sự kiện FormClosed (tên là HomePage_FormClosing)
                            this.Invoke(new Action(() =>
                            {
                                // Chỉ đóng form một lần
                                if (!this.IsDisposed)
                                {
                                    this.Close();
                                }
        ...
        {
            Form_QuestionUser forma = new Form_QuestionUser(questionJson, username);
            forma.Show();
        }
