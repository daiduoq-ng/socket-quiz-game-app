...
namespace NT106
{
    public partial class Server : Form
    {
        ...
        {
            string jsonFilePath = "questions_1.json";
        ...
        {
            string jsonFilePath = "questions_2.json";
            List<QuestionModel> questions = null;
            try
            {
                questions = LoadQuestionsFromJson(jsonFilePath);
        ...
        {
            string jsonFilePath = "questions_3.json"; 
            List<QuestionModel> questions = null;
            try
            {
                questions = LoadQuestionsFromJson(jsonFilePath);
            ...
            try
            {
                while (true)
                {
                    ...
                    if (receivedBytes > 0)
                    {
                        ...
                        {
                            response = ChangePassword((string)request.username,(string)request.password);
                        }
                        else if (request.type == "forgotPassword")
                        {
                            response = ForrgotPassword((string)request.email);
                        }
        ...
        private string ChangePassword(string username, string newPassword)
        {
            ...
            try
            {
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    ...
                    if (rowsAffected > 0)
                    {
                        var userPass = new { status = "success", message = "Thay đổi mật khẩu thành công!" };
            ...
            {
                response = JsonConvert.SerializeObject(new { status = "error", message = "Error updating password: " + ex.Message });
            }
        ...
        {
            string query = @"
            SELECT TOP 3 tk.UserName, tk.ProfileImage, qr.Score
            FROM QuizResult qr
            JOIN TaiKhoan tk ON qr.UserId = tk.UserId
            WHERE qr.QuizId = (SELECT MAX(QuizId) FROM QuizResult)
        ...
        private string ForrgotPassword(string email)
        {
            string query = "SELECT UserName, PassWord From TaiKhoan WHERE Email = @Email";
            string response;
            try
            {
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    ...
                    {
                        response = JsonConvert.SerializeObject(new { status = "error", message = "Email not found" });
                    }
                    connection.Close();
                }
        ...
        string HandleRegister(string username, string password, string email, byte[] profileImage)
        {
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                {
                    Console.WriteLine("Email đã tồn tại: " + email); //log giá trị email
                ...
                command.Parameters.AddWithValue("@Password", password);
                command.Parameters.AddWithValue("@Email", email);
                command.Parameters.AddWithValue("@ProfileImage", SqlDbType.VarBinary).Value = profileImage; // Sử dụng tham số mới để truyền dữ liệu hình ảnh
        ...
        {
            InitializeCountdownTimer();
            timeDown = int.Parse(tbCountDown.Text); 
        ...
        private void Server_Load(object sender, EventArgs e)
        {
            ...
            label3.Text = "IP Server: " + ipv4Address;
        }
    }
}
